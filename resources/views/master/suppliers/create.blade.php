...
@section('content')
    <div class="container-fluid px-0 px-lg-3">
        <header
            ...
            <div>
                <h1 class="fw-bold mb-1">Tambah Supplier</h1>
                <p class="text-muted mb-0">Lengkapi informasi pemasok baru untuk kebutuhan pengadaan proyek.</p>
            </div>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4 p-lg-5">
                ...
                <form action="{{ route('suppliers.store') }}" method="POST" class="row g-4">
                    @csrf
                    ...
                    <div class="col-md-6">
                        ...
                        <select id="rating" name="rating" class="form-select @error('rating') is-invalid @enderror">
                            @foreach (range(0, 5) as $ratingOption)
                                <option value="{{ $ratingOption }}" @selected((string) $ratingOption === old('rating'))>
                                    {{ $ratingOption }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <textarea id="address" name="address" rows="4"
                            ...
                            placeholder="Alamat lengkap kantor atau gudang">{{ old('address') }}</textarea>
                        @error('address')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
